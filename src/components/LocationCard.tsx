  ...
  return (
    <article className="flex flex-col border border-line bg-paper p-5">
      ...
      <h3 className="mt-3 font-display text-2xl leading-tight tracking-tight">
        <Link href={`/locations/${location.slug}`} className="hover:text-teal">
          {location.street}
        </Link>
      </h3>
...
export function LocationRow({ location }: { location: Location }) {
  return (
    <li className="grid gap-2 border-b border-line py-4 md:grid-cols-[1fr_1.2fr_auto] md:items-center">
      <p className="text-[0.7rem] uppercase tracking-[0.16em] text-teal">
        {location.neighborhood}
      </p>
      <div>
        <Link
          href={`/locations/${location.slug}`}
          className="font-display text-xl tracking-tight hover:text-teal"
        >
          {location.street}
        </Link>
        <p className="text-sm text-ink/60">Houston, TX {location.zip}</p>
      </div>
